...
$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
		url:path+"/bill/providerList.do",//请求的url
	 ...
	 * jquery的方法传递
	 */
    creationDate.on("blur",function(){
		if(creationDate.val() != null && creationDate.val() != ""){
			validateTip(creationDate.next(),{"color":"green"},imgYes,true);
		}else{
			validateTip(creationDate.next(),{"color":"red"},imgNo+" 日期不能为空，请重新选择",false);
		}
	}).on("focus",function(){
		//显示友情提示
		validateTip(creationDate.next(),{"color":"#666666"},"* 请选择日期",false);
	}).focus();
	...
	addBtn.on("click",function(){
		if(creationDate.attr("validateStatus") != "true"){
            creationDate.blur();
